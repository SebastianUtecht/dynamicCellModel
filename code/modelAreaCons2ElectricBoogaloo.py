...
class Simulation:
    ...
    def __init__(self, sim_dict):
        ...
        self.target_A       = sim_dict['target_A']              # Target area for the cells. Only relevant if penalize_A is not 0
        self.penalize_A     = sim_dict['penalize_A']            # Penalty strength for area deviation
        ...
        if isinstance(self.alpha_params[0], list):
            ...
            self.type0_gamma_par_free = self.gamma_params[0][0][1] == 'free'
            self.type0_gamma_perp_free = self.gamma_params[0][1][1] == 'free'

            # Cell type 1
            self.type1_alpha_par_free = self.alpha_params[1][0][1] == 'free'
            self.type1_alpha_perp_free = self.alpha_params[1][1][1] == 'free'
            self.type1_gamma_par_free = self.gamma_params[1][0][1] == 'free'
            self.type1_gamma_perp_free = self.gamma_params[1][1][1] == 'free'

        else:
            self.type0_alpha_par_free = self.alpha_params[0][1] == 'free'
            self.type0_alpha_perp_free = self.alpha_params[1][1] == 'free'
            self.type0_gamma_par_free = self.gamma_params[0][0] == 'free'
            self.type0_gamma_perp_free = self.gamma_params[0][1] == 'free'

            self.type1_alpha_par_free = False
            self.type1_alpha_perp_free = False
            self.type1_gamma_par_free = False
            self.type1_gamma_perp_free = False
        ...
        self.gamma_par_bool_lst     = [self.type0_gamma_par_free, self.type1_gamma_par_free]
        self.gamma_perp_bool_lst    = [self.type0_gamma_perp_free, self.type1_gamma_perp_free]
    ...
    def get_neighbors_vor(self, x, p, q, gamma_par, gamma_perp, k):
        ...
        z_mask = self.find_true_neighbours(d, dx, idx,  p, q, gamma_par, gamma_perp)
    ...
    def planes_bound(self, pos):
        ...
        bound_dists = torch.abs(pos[:,-1]) - self.bound_cur_ext/2
    ...
    def potential(self, x, p, q, p_mask, alpha_par, alpha_perp, gamma_par, gamma_perp, d, dx, idx, z_mask):
        ...
        Parameters:
            ...
            gamma_par (torch.Tensor): The parallel gamma (elongation) parameter.
            gamma_perp (torch.Tensor): The perpendicular gamma (elongation) parameter.
            d (torch.Tensor): The distances to the neighbors.
            dx (torch.Tensor): The normalized displacement vectors to the neighbors.
        ...
        perp_dir = perp_dir / (torch.linalg.norm(perp_dir, dim=2, keepdim=True) + 1e-8)
    ...
    def init_simulation(self, x, p, q, p_mask, alpha_par, alpha_perp, gamma_par, gamma_perp):
        ...
        Parameters:
            ...
            gamma_par (np.ndarray): The parallel gamma (elongation) parameter.
            gamma_perp (np.ndarray): The perpendicular gamma (elongation) parameter.

        Returns:
            Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]: The initialized tensors for cell positions, apicobasal polarities, planar cell polarities, and the particle mask.
            OR
            Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, None]: If p_mask is None, the last element is None.
        ...
        gamma_par = torch.tensor(gamma_par, requires_grad=True, dtype=self.dtype, device=self.device)
        gamma_perp = torch.tensor(gamma_perp, requires_grad=True, dtype=self.dtype, device=self.device)
        ...
        return x, p, q, p_mask, alpha_par, alpha_perp, gamma_par, gamma_perp # Returning the goods.
    ...
    def time_step(self, x, p, q, p_mask, alpha_par, alpha_perp, gamma_par, gamma_perp, tstep):
        """
        ...
        self.division, x, p, q, p_mask, self.beta, alpha_par, alpha_perp, gamma_par, gamma_perp = self.cell_division(x, p, q, p_mask, alpha_par, alpha_perp, gamma_par, gamma_perp)
        # self.apoptosis, x, p, q, p_mask, self.beta, alpha_par, alpha_perp, gamma_par, gamma_perp = self.cell_apoptosis(x, p, q, p_mask, alpha_par, alpha_perp, gamma_par, gamma_perp)
        ...
        with torch.no_grad():
            for i,eta in enumerate(self.eta_lst):
                ...
                if self.gamma_par_bool_lst[i]:
                    gamma_par[p_mask == i] += -gamma_par.grad[p_mask == i] * self.dt + eta * torch.empty(*gamma_par[p_mask == i].shape, dtype=self.dtype, device=self.device).normal_() * self.sqrt_dt
                    gamma_par[p_mask == i] = torch.clamp(gamma_par[p_mask == i], self.gamma_range[0], self.gamma_range[1])

                if self.gamma_perp_bool_lst[i]:
                    gamma_perp[p_mask == i] += -gamma_perp.grad[p_mask == i] * self.dt + eta * torch.empty(*gamma_perp[p_mask == i].shape, dtype=self.dtype, device=self.device).normal_() * self.sqrt_dt
                    gamma_perp[p_mask == i] = torch.clamp(gamma_perp[p_mask == i], self.gamma_range[0], self.gamma_range[1])
        ...
        if any(self.gamma_par_bool_lst):
            gamma_par.grad.zero_()
        if any(self.gamma_perp_bool_lst):
            gamma_perp.grad.zero_()
    ...
    def simulation(self, x, p, q, p_mask, alpha_par, alpha_perp, gamma_par, gamma_perp):
        ...
        x, p, q, p_mask, alpha_par, alpha_perp, gamma_par, gamma_perp = self.init_simulation(x, p, q, p_mask, alpha_par, alpha_perp, gamma_par, gamma_perp)
        ...
        while True:
            ...
            if tstep % self.yield_every == 0 or len(x) > self.max_cells:    #Yield data if we are at a 'yield step' or if we have too many cells and the simulation is aborted
                ...
                gammagamma_par = torch.exp(gamma_par.detach().clone())
                gammagamma_par = gammagamma_par.to("cpu").numpy()

                gammagamma_perp = torch.exp(gamma_perp.detach().clone())
    ...
    def cell_division(self, x, p, q, p_mask, alpha_par, alpha_perp, gamma_par, gamma_perp):
        ...
        Returns:
            Tuple[bool, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor or None, torch.Tensor]: A tuple containing:
                ...
                - gamma_par (torch.Tensor): The parallel gamma (elongation) parameter with new cells.
                - gamma_perp (torch.Tensor): The perpendicular gamma (elongation) parameter with new cells.
        """

        beta = self.beta            

        if torch.sum(beta) < 1e-8:              # If division probabilities are negligible no division occurs
            return False, x, p, q, p_mask, beta, alpha_par, alpha_perp, gamma_par, gamma_perp
        ...
        if torch.sum(events) > 0:
            with torch.no_grad():
                ...
                gamma_par0 = gamma_par[idx]
                gamma_perp0 = gamma_perp[idx]
                ...
                gamma_par = torch.cat((gamma_par, gamma_par0))
                gamma_perp = torch.cat((gamma_perp, gamma_perp0))
        ...
        gamma_par.requires_grad = True
        gamma_perp.requires_grad = True

        return division, x, p, q, p_mask, beta, alpha_par, alpha_perp, gamma_par, gamma_perp      #Returning the goods.

    # def cell_apoptosis(self, x, p, q, p_mask, alpha_par, alpha_perp, gamma_par, gamma_perp):
    #     """
...
def run_simulation(sim_dict):
    ...
    if isinstance(data, dict):
        ...
        alpha_params = sim_dict['alpha_params']
        gamma_params = sim_dict['gamma_params']
        p_mask, x, p, q, alpha_par, alpha_perp, gamma_par, gamma_perp = data_gen_function(*data_gen_parameters,
    ...
    runner = sim.simulation(x, p, q, p_mask, alpha_par, alpha_perp, gamma_par, gamma_perp)    # Making a runner
...
def make_random_sphere(N, type0_frac , radius=30, alpha_params=None, gamma_params=None):
    ...
    gamma_par = np.zeros(N)
    gamma_perp = np.zeros(N)
